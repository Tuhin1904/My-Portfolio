...
export default function UrlShortenerPage() {
  ...
  const [linkToDelete, setLinkToDelete] = useState<ShortUrlItem | null>(null);
  ...
  const handlePresetExpiry = (hours: number) => {
    if (hours === 0) {
      setExpiresAt('');
      return;
    }
  ...
  const confirmDelete = async (id: string) => {
    try {
      ...
      if (response && response.success) {
        ...
        toast.error(response?.message || 'Failed to delete short URL');
      }
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-white flex flex-col transition-colors duration-200">
      <div className="flex-1 relative overflow-hidden py-10 px-4 md:px-8 max-w-6xl mx-auto w-full">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider flex items-center justify-between">
                  ...
                  {expiresAt && (
                    <button
                      ...
                      className="text-[11px] text-red-500 hover:text-red-600 font-medium underline"
                    >
                      Clear
                    </button>
                  )}
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-950/80 border border-gray-300 dark:border-white/10 rounded-xl text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all text-sm [color-scheme:light] dark:[color-scheme:dark] cursor-pointer"
                  />
                </div>
                {/* Quick Presets */}
                <div className="flex items-center gap-1.5 pt-0.5">
                  <button
                    ...
                    className="px-2.5 py-1 text-[11px] rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-indigo-500/10 hover:text-indigo-600 dark:hover:text-indigo-400 text-gray-600 dark:text-gray-300 font-medium transition-colors border border-gray-200 dark:border-white/10"
                  >
                    +24h
                  </button>
                  <button
                  ...
                  <button
                    type="button"
                    onClick={() => handlePresetExpiry(720)}
                    className="px-2.5 py-1 text-[11px] rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-indigo-500/10 hover:text-indigo-600 dark:hover:text-indigo-400 text-gray-600 dark:text-gray-300 font-medium transition-colors border border-gray-200 dark:border-white/10"
                  >
                    +30d
                  </button>
                  <button
                    type="button"
                    onClick={() => handlePresetExpiry(0)}
                    className="px-2.5 py-1 text-[11px] rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-red-500/10 hover:text-red-500 text-gray-600 dark:text-gray-300 font-medium transition-colors border border-gray-200 dark:border-white/10"
                  >
                    Never
                  </button>
                </div>
                    ...
                    >
                      <div className="space-y-1 min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          >
                            {isExpired ? 'Expired' : item.isActive ? 'Active' : 'Disabled'}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-4 text-[11px] text-gray-500 pt-1 flex-wrap">
                          ...
                          <span className="flex items-center gap-1">
                            <Clock className="w-3 h-3 text-gray-400 dark:text-gray-500" /> Created {new Date(item.createdAt).toLocaleDateString()}
                          </span>
                          {item.expiresAt ? (
                            <span
                      ...
                      <div className="flex items-center gap-2 self-end md:self-center">
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 transition-colors border border-gray-300 dark:border-white/10 cursor-pointer"
                          title="View QR Code"
                        >
        ...
        <AlertDialog open={!!linkToDelete} onOpenChange={(open) => !open && setLinkToDelete(null)}>
          <AlertDialogContent className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white">
            <AlertDialogHeader>
              <AlertDialogTitle className="text-gray-900 dark:text-white font-bold flex items-center gap-2">
                <Trash2 className="w-5 h-5 text-red-500" /> Are you sure?
              </AlertDialogTitle>
              <AlertDialogDescription className="text-gray-600 dark:text-gray-400 text-xs">
                This will permanently delete short link{' '}
                <span className="font-mono font-bold text-indigo-600 dark:text-indigo-400">
                  /{linkToDelete?.shortCode}
                </span>. It will immediately stop working.
              </AlertDialogDescription>
            </AlertDialogHeader>

            <AlertDialogFooter>
              <AlertDialogCancel onClick={() => setLinkToDelete(null)} className="cursor-pointer">
                Cancel
              </AlertDialogCancel>
              ...
              <AlertDialogAction
                onClick={() => {
                  if (linkToDelete) {
                    confirmDelete(linkToDelete._id);
